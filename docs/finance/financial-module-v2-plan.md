---
id: financial-module-v2-plan
title: Financial Module v2 Plan
...
The module depends on exactly **two functions** from `@arrival-atlas/shared-services`:
...
The `@arrival-atlas/core` package.json references Jest but no test files exist. **No golden fixtures, no regression suite, no property-based tests.**
...
- run: npm run test -w @arrival-atlas/shared-services -- financial
- run: npm run test -w @arrival-atlas/modules -- financial-reality
```
...
- [ ] Create `docs/finance/financial-module-v2-plan.md` review ticket
