...
- **G1** — Web types unified under `@arrivalos/product-contract` (no local domain mirrors)
- **G2** — `@arrivalos/core` fully removed from web dependency graph; CI enforces via `web-package-boundary.test.ts`
...
| **G2** No runtime imports in web | **PASS** | Zero `@arrivalos/core` imports; removed from `apps/web/package.json`; `web-package-boundary.test.ts` |
...
- New package `@arrivalos/ui-contract` (language, theme, branding)
- Core re-exports UI primitives from ui-contract (no local branding file)
- Web imports exclusively from `@arrivalos/product-contract`
...
| Package | Tests |
|---------|-------|
| `@arrivalos/ui-contract` | 1 |
| `@arrivalos/product-contract` | 59 |
| `@arrivalos/api` | 174 |
| `@arrivalos/web` | 8 |
| **Monorepo total** | **430+** |
