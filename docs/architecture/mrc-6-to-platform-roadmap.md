...
**Supersedes (partially):** [Module Runtime Evolution Roadmap](./module-runtime-evolution-roadmap.md) Phases MRC-6/MRC-7 naming — governance kernel (MRC-6 debt closure) is complete; this document defines the **next** tranche.

**Bound by:**

- [MRC ADL v1.0](./mrc-adl-architecture-decision-layer.md)
- [Module Runtime Contract v1.0](./module-runtime-contract-v1-specification.md)
...
| Regression suite | ✅ | 45/45 `@arrivalos/module-runtime`, 155/155 `@arrivalos/api` |
...
|---------|----------|---------------------|
| `GovernedModuleRegistry` | `@arrivalos/module-runtime/governance` | ✅ not exported via API |
| `globalRegistry` | `@arrivalos/core` | ⚠️ still used at bootstrap only |
...
| `GET /api/modules/:id/schema` | ❌ | — | Listed in `docs/CURRENT_STATE.md` P1; never implemented |
...
|---|-----------|--------------|
| E1 | UI can list/describe modules without registry imports | Contract test: web package has zero `@arrivalos/core` registry imports |
...
**Deliverable:** `@arrivalos/module-sdk`
...
**Deliverable:** `docs/architecture/module-versioning-policy.md` + semver gate in `validateModuleRegistration()`.
...
| G2 | No `@arrivalos/core` or `@arrivalos/module-runtime` imports in `apps/web` |
...
|----------|----------------------|--------------|
| [module-runtime-evolution-roadmap.md](./module-runtime-evolution-roadmap.md) | MRC-6 = UI Snapshot Integration | **Renamed:** UI snapshot work moves to **this roadmap Phase 5** |
| Same doc | MRC-7 = Runtime Governance | **Done early** as MRC-5 debt closure + MRC-6 governance kernel |
| [mrc-adl-architecture-decision-layer.md](./mrc-adl-architecture-decision-layer.md) | §7 UX/snapshot migration at "MRC-6" | Deferred — explicit scope of **Phase 5 here** |
