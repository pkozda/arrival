...
**Hard rule:** In `NODE_ENV=test` and `ARRIVALOS_IAM_STRICT=true` (dev), unclassified routes throw `UNCLASSIFIED_ROUTE` and fail the request. Emit `IAMEventType.ROUTE_UNCLASSIFIED`.
...
| **3 — Route audit** | Dev | Enable `ARRIVALOS_IAM_STRICT`; log/fix unclassified routes |
...
```
ARRIVALOS_IAM_STRICT=true   # dev/test: unclassified routes fail
ARRIVALOS_IAM_STRICT=false  # prod: log only until confidence (short window)
```
...
- [ ] Unclassified route → hard fail when `ARRIVALOS_IAM_STRICT=true`
