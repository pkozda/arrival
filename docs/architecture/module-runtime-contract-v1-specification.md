...
- [Module Runtime Contract v1.0 — Evolution Roadmap](./module-runtime-evolution-roadmap.md)
...
The roadmap ([Module Runtime Evolution Roadmap](./module-runtime-evolution-roadmap.md)) describes *what* to build and in what order. This document defines *how* the contract looks: TypeScript interfaces, pipeline invariants, and enforcement rules.
...
All types below are **normative** for Module Runtime Contract v1.0. Implementations may live in a new `@arrivalos/module-runtime` package (Phase MRC-1).
...
```typescript
import type { SupportedLanguage, DataProvenanceEntry } from '@arrivalos/core';
...
| **SR-5** | Frontend has no imports from `@arrivalos/profile`, `@arrivalos/modules`, or DPSS types |
...
- [ ] `@arrivalos/module-runtime` package exists with normative types from §3
